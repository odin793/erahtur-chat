...
(function() {
	...
	window.addEvent("domready", function() {
		...
		fx = {
			...
			bottom_height: new Fx.Tween(bottom, {property: "height", duration: 10}),
		};
	});
...
//			NonverBlaster
				...
				playerpath: 'NonverBlaster-hover/Source/NonverBlaster.swf',	// Path to NonverBlaster.swf
...
//			if (options.overflow) image.setStyles({overflow: options.overflow});
		...
		mediaHeight = image.offsetHeight+bottom.offsetHeight + 20;

		if (mediaHeight >= top+top) { mTop = -top } else { mTop = -(mediaHeight/2) };
		if (mediaWidth >= left+left) { mLeft = -left } else { mLeft = -(mediaWidth/2) };
		if (options.resizeOpening) {fx.resize.start({width: mediaWidth, marginTop: mTop-margin, marginLeft: mLeft-margin});
