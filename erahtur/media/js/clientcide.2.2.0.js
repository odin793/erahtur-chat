...
//This lib: http://www.clientcide.com/js/build.php?excludeLibs[]=mootools-core&excludeLibs[]=mootools-more&require[]=TabSwapper&compression=none
...
//	onBackground: $empty
	...
	show: function(i){
		...
		/*
		this.showSection(i);
		if (this.we_changed_tab) {
		    if (this.options.cookieName) {
		    Cookie.dispose(this.options.cookieName);
		    console.log('we will save cookie now, as the next expression is', this.we_changed_tab);
		    this.save(i);
		}
		else {
		console.log('we dont save cookie now', this.we_changed_tab);
	    };
	    */
		return this;
	},
	...
	showSection: function(idx) {
		...
		if (this.now != idx) {
			...
			this.we_changed_tab = true;
			//console.log('from showSection', this.we_changed_tab);
			this.now = idx;
			this.fireEvent('onActive', [idx, sect, tab]);
		}
		else {
		    this.we_changed_tab = false;
		};
